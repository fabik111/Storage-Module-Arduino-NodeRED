...
function getFlows() {
    ...
    const StorageApiClient = settings.functionGlobalContext.arduinoConnectionManager.apiStorage;
    return StorageApiClient.readFlow().then(function(result){
        if(result==='error'){
            return [];
        }
        return result;
...
function saveFlows(flows) {
    ...
    const StorageApiClient = settings.functionGlobalContext.arduinoConnectionManager.apiStorage;
    return StorageApiClient.writeFlow(body).then(function(result){
        return result;
    });/*
